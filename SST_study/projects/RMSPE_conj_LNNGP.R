...
ind_C.ho_X <- rep(1:N_test, each = M)
ind_C.ho_Y <- c(t(nn.mod.ho$nn.idx))
...
D_ho <- AD_ho[, M + 1]
C.p <- cbind(X_test, sparseMatrix(ind_C.ho_X, ind_C.ho_Y,
                                x = c(t(AD_ho[, -(M + 1)]))))
L.gamma <- dim(C.p)[2]
proc.time() - t
...
RMSPEfun <- function(s){
    ...
    Y.ho.m <- C.p %*% gamma_sim[1:L.gamma]
...
test_post_mean <- C.p %*% gamma_hat[1:L.gamma]
