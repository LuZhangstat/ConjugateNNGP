# RMSPE 

setwd("./simulation")
rm(list = ls())
library("sp")
library("rstan")
library("Matrix")
library("fields")
load("./data/sorted_x_order/nngp_10.RData")
load("./results/latent_NNGP")

M = 10

# get posterior samples
post_sim <- extract(samples_w)
n_run <- 500; L <- length(post_sim$sigmasq)
set.seed(1)
sample_ind <- sample(L, n_run)

Y_pred <- Matrix(0, nrow = (N_total - N), ncol = n_run)

for (l in (1):(N_total - N)){
  cat(l, "th new sample: \n")
  NNind_l <- sort(order(
    rdist(coords_test[c(1,l),], m.c$coords.ord)[-1,])[1:M])
  NNdist <- dist(rbind(coords_test[l,], m.c$coords.ord[NNind_l, ]))
  NNdistv <- c(NNdist)[1:M]
  NNdistM <- as.matrix(NNdist)[-1, -1]
  
  for (ind in 1:n_run){
    s <- sample_ind[ind]
    beta_sim <- post_sim$beta[s,]
    sigmasq_sim <- post_sim$sigmasq[s]
    tausq_sim <- post_sim$tausq[s]
    phi_sim <- post_sim$phi[s]
    w_sim <- post_sim$w[s, ]
    
    w_b1_sim <- w_sim[NNind_l] - beta_sim[1]
    
    NNcorM <- exp(- phi_sim * NNdistM )
    chol_NNcorM <- t(chol(NNcorM))
    NNcorV <- exp(- phi_sim * NNdistv)
    v <- forwardsolve(chol_NNcorM, NNcorV)
    
    var_l <- sigmasq_sim * (1 - sum(v^2))
    mean_l <- beta_sim[1] + t(backsolve(t(chol_NNcorM), v)) %*% w_b1_sim
    
    w_new <- mean_l + rnorm(1) * sqrt(var_l)
    
    Y_pred[l, ind] <- X_test[l, 2]*beta_sim[2] + w_new + 
      rnorm(1)* sqrt(tausq_sim)
  }
}

MSPE <- mean((Y_test - rowMeans(Y_pred))^2)
RMSPE <- sqrt(MSPE); RMSPE

save(Y_pred, file = "compare/latent_NNGP/Y_pred_MSPE.RData")

#----------------------- MSE of random effect --------------------#

set.seed(1)
n_run <- 500; L <- length(post_sim$sigmasq)
sampleind <- sample.int(L, n_run)


MSE.w <- mean(colSums((t(post_sim$w[sampleind,]- post_sim$beta[sampleind,1]) - 
w[m.c$ord])^2)); MSE.w





