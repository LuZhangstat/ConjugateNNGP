...
for (s in sampleind){
  ...
  cat("deviance: ", deviance[simind], "\n")
  
}
proc.time() - t

save(score, deviance, 
     file = "compare/model2_full/score_dev1.RData")

load(file = "compare/model2_full/score_dev1.RData")

mean(score)
mean(deviance)
quantile(deviance, c(0.5, 0.025, 0.975))
